...
const XPATH_ESPECIALIDADE_NOME = '//*[@id="formulario:viewCadastro:tblDataEspecialidade:0:j_id240"]';
const XPATH_ESPECIALIDADE_AMBULATORIO = '//*[@id="formulario:viewCadastro:tblDataEspecialidade:0:j_id242"]';
const XPATH_ESPECIALIDADE_ATIVO = '//*[@id="formulario:viewCadastro:tblDataEspecialidade:0:j_id245"]';
...
async function capturarDadosEspecialidade(page) {
    ...
    try {
        ...
        await clicar(page, '//*[@id="formulario:viewCadastro:abaEspec_lbl"]');
        await sleep(500);
...
async function adicionarEspecialidade(page, especialidadeValor, fazAmbulatorio) {
    ...
    try {
        ...
        const selectXPath = '//*[@id="tabelaEspecialidades"]/tbody/tr[1]/td[2]/select';
...
async function adicionarGrupo(page, xpathItem, nomeGrupo) {
    ...
    // com um clique sintético, mas só habilita os botões com um evento de mouse real.
    ...
    try {
        ...
        // Diagnóstico: captura o estado real da linha do item e do container do botão no momento
        // da falha - se o clique real também não habilita o botão, precisa ver se o item ficou
        // mesmo selecionado (classe de destaque) ou se o botão está desabilitado por outro motivo
        // (ex: permissão do setor do agente, não mecânica de clique).
        const diagnostico = await page.evaluate((xpathItemDiag, xpathBotao) => {
            const itemResult = document.evaluate(xpathItemDiag, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
            const td = itemResult.singleNodeValue;
            const linha = td ? td.closest('tr') : null;
            const botaoResult = document.evaluate(xpathBotao, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
            const botao = botaoResult.singleNodeValue;
            // Sobe até o container que agrupa todos os botões do shuttle (copy/move/etc) para achar
            // o irmão visível que substitui o "pickdiscopy" escondido depois da seleção.
            const containerBotoes = botao ? botao.parentElement : null;
            return {
                classeLinha: linha ? linha.className : null,
                htmlBotao: botao ? botao.outerHTML : null,
                htmlContainerBotoes: containerBotoes ? containerBotoes.outerHTML : null
            };
        }, xpathItem, XPATH_CONTAINER_BOTAO_ADICIONAR_GRUPO);
        console.log(`   🔬 Classe da linha "${nomeGrupo}" na falha: ${diagnostico.classeLinha}`);
        console.log(`   🔬 HTML do botão na falha: ${diagnostico.htmlBotao}`);
        console.log(`   🔬 HTML do container de botões na falha: ${diagnostico.htmlContainerBotoes}`);

        const screenshotPath = path.join(__dirname, 'logs', `debug_botao_desabilitado_${Date.now()}.png`);
        try {
            await page.screenshot({ path: screenshotPath, fullPage: true });
            console.log(`   📸 Screenshot de diagnóstico salvo em: ${screenshotPath}`);
        } catch (screenshotError) {
            console.log(`   ⚠️ Falha ao salvar screenshot de diagnóstico: ${screenshotError.message}`);
        }
    ...
    try {
        ...
        const screenshotPath = path.join(__dirname, 'logs', `debug_grupo_${nomeGrupo.replace(/[^A-Z0-9]/gi, '_')}_${Date.now()}.png`);
        try {
            await page.screenshot({ path: screenshotPath, fullPage: true });
            console.log(`   📸 Screenshot de diagnóstico salvo em: ${screenshotPath}`);
        } catch (screenshotError) {
...
// uma vez (mais robusto do que percorrer pick:1, pick:2... por índice, que arrisca parar antes
// da hora se algum índice não for sequencial). Igual à Especialidade, precisa clicar na aba
// "Grupos" antes da lista carregar completa.
...
// Salva/confirma as alterações - reaproveita confirmarAlteracaoComSenha porque o botão
// "Alterar" aqui abre o mesmo modal de senha usado em todo o resto do cadastro (troca de
// email, entidade/setor); a versão anterior clicava só em "Alterar" e nunca preenchia a senha.
async function salvarAlteracoesEspecialidade(page) {
    console.log("💾 Salvando alterações...");

    try {
        await confirmarAlteracaoComSenha(page);
...
async function enviarCodigoEmailVitae(email, codigo, nome) {
    ...
    const mailOptions = {
        from: `"Suporte TI - Alteração de Email" <${config?.email?.user || 'cleanleito@gmail.com'}>`,
        to: email,
...
async function preencherCampoPesquisa(page, tipo, valor) {
    ...
    // CPF para preservar a detecção de usuário INATIVO (CASO 1/CASO 2) já existente.
...
// Função para buscar usuário pelo CPF, Login ou Nome (já com a página pronta) - COM VERIFICAÇÃO DE STATUS E CAPTURA DE ESPECIALIDADE
async function buscarUsuarioVitae(from, valorBusca, tipoBusca = 'cpf', capturarEspecialidade = false) {
    const cpf = valorBusca;
    ...
    // 🔥 DECLARA A VARIÁVEL AQUI - NO ESCOPO DA FUNÇÃO
    let dadosEspecialidade = null;

    try {
        // Garante que a página está na tela de listagem/busca antes de pesquisar
        try {
        ...
        // Variáveis para capturar dados do Caso 2
        ...
        for (let tentativaGlobal = 1; tentativaGlobal <= MAX_TENTATIVAS_REFRESH; tentativaGlobal++) {
            ...
            if (pesquisaSucesso && (temBotaoEditar || nomeUsuarioCaso2)) {
                ...
                console.log(`   ❌ Tentativa global ${tentativaGlobal} falhou`);
        ...
        // 🔥 CASO 2: Usuário ATIVO mas sem botão de editar
        ...
        // 🔥 ========== CAPTURA DADOS DA ESPECIALIDADE (se solicitado) ==========
        if (capturarEspecialidade) {
            ...
            try {
                ...
                if (abaExiste) {
                    // 🔥 CHAMA A FUNÇÃO QUE CAPTURA TODAS AS ESPECIALIDADES
        ...
        // 🔥 RETORNO COM dadosEspecialidade (já declarada no início da função)
        return {
            encontrado: true,
...
async function alterarEntidadeSetor(page, entidadeValor, setorNomePadrao, password, garantirGrupoConsulta = false) {
    ...
    // 🔥 AGUARDA O MODAL FECHAR (sinal de sucesso)
    await sleep(3000);
    
    console.log(`   ✅ Entidade/Setor alterados com sucesso (Setor: ${opcaoSetor.nome})`);
}
...
async function transferirUnidadeUsuario(from, valorBusca, tipoBusca = 'cpf') {
    ...
    try {
        ...
        if (!loginAlvo) {
            // Tenta gerar um login a partir do nome (ex: "João Silva" -> "JOAOSILVA")
        ...
        await alterarEntidadeSetor(page, unidadeOrigemValor, SETOR_AMBULATORIO_NOME, senhaAgente);
                ...
                // 🔥 TENTA UMA SEGUNDA VEZ COM UM DELAY MAIOR
